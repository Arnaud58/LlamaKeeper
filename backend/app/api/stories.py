...
async def create_story(
    ...
    """Create a new story"""
    # First, validate that all character IDs exist
    if story.character_ids:
        ...
        existing_characters = character_result.scalars().all()
    ...
    # Add character associations if characters were provided
    if story.character_ids:
        for character_id in story.character_ids:
            character = await session.get(db_models.Character, character_id)
            if character:
                db_story.characters.append(character)
...
async def get_story(story_id: int, session: AsyncSession = Depends(get_async_session)):
    ...
    result = await session.execute(
        select(db_models.Story).where(db_models.Story.id == story_id)
    )
